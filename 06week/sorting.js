...
const stringsToNumbs = (numbers) => {
    ...
    for (i=0;i<numbers.length;i++){
        let ints = parseInt(numbers[i], 10)
        newArr.push(ints)
    }
    return newArr;
}
...
const sumEvens = stringsToNumbs(strNums).filter(x => x%2==0).reduce((total, amount) => total+amount)
console.log(sumEvens);

// Find the index of the first value when added to it's index = 512 (#ATX!!)
let nums = strNums
const atxFinder = (nums) => {
...
/* Create an array of the unique 'weather_state_name' values of the weather array. 
...
for (let j=0; j<weatherStates.length; j++) {
    if (result.indexOf(weatherStates[j] === -1)) {
        result.push(weatherStates[j]);
    }
}
...
const idealTemp = findTemp(weather);
